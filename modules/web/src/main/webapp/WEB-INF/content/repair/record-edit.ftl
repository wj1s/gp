...
<script language="JavaScript" type="text/javascript">
	...
	function refresh(){
		...
		for (var i in array){
			...
			html += '<td>'+array[i].name+'</td>';
			   if(array[i].empId!=null&&array[i].empId!='')
			   {
				    html+='<td class="empClass">'+array[i].empId+'</td>'
			    }else
				{
	...
	$(document).ready(function(){
		...
		<#if recordDetail.recordItems?exists>
		  <#list recordDetail.recordItems as recordItem>
		  item1 = {};
		  item1.id=${recordItem.id};
		  item1.name ='${recordItem.content}';
		  item1.examineRecord ='';
		  item1.measure ='';
		  item1.cycleName='${recordItem.cycleName}';
		  item1.type=${recordItem.type};
		  <#if recordItem.card?exists>
		    item1.type=0;
		    item1.id=${recordItem.card.id}
		  <#else>
		  item1.type=1;
		  </#if>
		  item1.repairName='<#if recordItem.persons?exists><#list recordItem.persons as person>${person.name} </#list></#if>';
		  item1.repairNameId='<#if recordItem.persons?exists><#list recordItem.persons as person>${person.id},</#list></#if>';
		   array.push(item1);
		   refresh();
		   </#list>
		</#if>
		...
		$('#recordForm').ajaxForm({
			...
			success: function(data){
				if (data['result'] == true){
					tjAlert('操作成功!');
				var url = '${request.contextPath}/repair/record!view.action';
...
<body>
    	...
    	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="work_info_table">
	                 <tr>
	                    <th colspan="4">
	                        <strong><span>编辑检修记录 </span></strong>
	                    </th>
	                </tr>
	                <tr>
	                    <td align="right">
	                    ...
	                    <td><input readonly="readonly" type="text" value="<#if recordDetail.group?exists>${recordDetail.group.name}</#if>"  id="groupId" />
	                     </td>
	                </tr>
	                <tr>
	                    <td align="right">
	                    ...
	                    <td colspan="3">
	                    	<input type="text" readonly="readonly"  id="examinePersons" name="examinePersons" size="40" maxlength="100" verify="检修人员|NotNull&&Length<50" value="<#if recordDetail.examinePersons?exists>${recordDetail.examinePersons}</#if> "/>
	                        <!-- input type="hidden" value="<#if curUser.group?exists><#list curUser.group.people as person>${person.id},</#list></#if>" id="examinePerpleIds"-->
	                    </td>
	                </tr>
	                <tr>
	                    <td align="right">
	                    ...
	                    <td>
	                        <input id="ddate"   name="ddate" type="text" ztype="date" verify="检修日期|NotNull" value="<#if recordDetail.ddate?exists>${recordDetail.ddate?string("yyyy-MM-dd")}</#if>"/>
	                    </td>
	                    <td align="right">
	                        <strong>检修时长：</strong>
	                    </td>
	                    <td>
	                        <input id="timeLength" name="timeLength"  type="text"  verify="检修时长|NotNull&&Int" value="<#if recordDetail.timeLength?exists>${recordDetail.timeLength?c}</#if>"/>分钟
	                    </td>
	                </tr>
	                 <tr>
	                    <td align="right">
	                    ...
	                    <td>
	                        <input id="security" name="security"   type="text" verify="安全员|NotNull&&Length<20" value="<#if recordDetail.security?exists>${recordDetail.security}</#if>"/>
	                    </td>
	                    <td align="right">
	                        <strong>负责人：</strong>
	                    </td>
	                    <td>
	                        <input id="principal"  name="principal" type="text" verify="负责人|NotNull&&Length<20" value="<#if recordDetail.principal?exists>${recordDetail.principal}</#if>"/>
	                    </td>
	                </tr>
	                <tr>
	                    <td align="right">
	                    ...
	                    <td colspan="7">
	                        <textarea id="measure"  rows="2" cols="60" name="measure" verify="安全措施|NotNull&&Length<256"><#if recordDetail.measure?exists>${recordDetail.measure}</#if></textarea>
	                    </td>
	                </tr>
	                <tr>
	                    <td align="right">
	                    ...
	                    <td colspan="7">
	                        <textarea id="examineRecord"  rows="2" cols="60" name="examineRecord" verify="检修记录|NotNull&&Length<512"><#if recordDetail.examineRecord?exists>${recordDetail.examineRecord}</#if></textarea>
	                    </td>
	                </tr>
	                <tr>
	                    <td align="right">
	                    ...
	                    <td colspan="7">
	                        <textarea id="test" rows="2" cols="60"   name="test" verify="试机情况|NotNull&&Length<256"><#if recordDetail.test?exists>${recordDetail.test}</#if></textarea>
	                    </td>
	                </tr>
	                </table>
	                </div>
	              <div align="right">
	    		<a title="添加检修项目" url="${request.contextPath}/repair/record!itemInputAjax.ajax" ztype="popUp" zwidth="600"><button ztype="button" type="button">添加检修项目</button></a><a title="添加临时检修项目" url="${request.contextPath}/repair/record!itemTemp.ajax" ztype="popUp" zwidth="600"><button ztype="button" type="button">添加临时检修项目</button></a><button ztype="button"  type="button">删除检修项目</button><a title="变更检修人员" id="changePersonA" url="${request.contextPath}/common/person-ex!getUserUsedAjax.ajax" ztype="popUp" zwidth="500"></a><button ztype="button" type="button">变更检修人员</button>
	    	      </div>
	        <div>
