...
//					ruleGroup += "]}";
					...
					//taiji自定义查询条件
					var ruleGroup = 'taijiFilter=';
					var gi=0;
					$.each(sdata,function(i,n){
						if ($('input[name="'+i+'"]').attr('zinitDate') != undefined){
							if (i.indexOf('_') == -1){
								ruleGroup += i + " = '" + n + "' and ";
							}else {
								ruleGroup += i.replace('_','.') + " = '" + n + "' + and ";
							}
						}else {
			    			var index = i.indexOf('_');
			    			var checkboxName = i.substring(0, index)+'_id';
							if ($('input[name="'+checkboxName+'"]').length > 0 && $('input[name="'+checkboxName+'"]:first').attr('type') == 'checkbox'){
								//没做呢
							}else {
								if (i.indexOf('_') == -1){
									ruleGroup += i + " like '%" + n + "%' and ";
								}else {
									ruleGroup += i.replace('_','.') + " like %" + n + "% and ";
								}
							}
						}
						gi++;
					});
					ruleGroup=ruleGroup.substring(0, ruleGroup.lastIndexOf("and"));
